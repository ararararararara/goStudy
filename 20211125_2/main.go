...
func ChangeData(d Data) { //매개변수로 Data
	d.value = 100
	d.data[100] = 200
}
...
func main() {
	...
	//포인터는 메모리 주소를 값으로 가지기 때문에..숫자..뭐 이런거 안됨

	var a int = 100
	p = &a //변수a의 메모리 주소를 포인터 변수 p에 대입
	...
	*p = 300
	...
	//0이긴 하지만 유효하지 않는 메모리 주소값,->어떠한 공간도 가리키고 있지않다.
	...
	//그래서 왜씀?이런걸?
	//대입이나 인수전달시 값을 복사하기 때문에 성능상에 이슈발생.다른공간에 복사하기때문에
	//변경사항이 적용 X
