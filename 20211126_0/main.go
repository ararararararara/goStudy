...
func main() {
	...
	//string,rune슬라이스 타입인 []rune타입은 상호 타입변환이 가능하다.
	//슬라이스는 길이가 변할수 있는 배열(가변적이다.)
	var string3 string = "Hello World"
	var runes = []rune{72, 101, 108, 108, 111, 32, 87, 111, 114, 108, 100}
	fmt.Println(string3)
	fmt.Println(string(runes)) //타입변환을 할 경우 rune배열의 각요소에는 문자열의 각 글자가 대입이 된다.
	//이걸이용해서 문자열의 글자수를 알수 있음.

	var string4 string = "Hello 월드"
	var runes1 = []rune(string4)                //[]rune타입으로 변환
	fmt.Println("string4 타입의 길이", len(string4)) //문자열의 바이트 길이가 반환
	fmt.Println("runes1 타입의 길이", len(runes1))   //각 글자들로 이루어진 배열로 반환
